...
RegisterNetEvent('nekoac:clientAction', function(action)
    local ped = PlayerPedId()
    ...
    elseif action == 'heal' then
        SetEntityHealth(ped, NekoAC.maxHealth or 200)
        SetPedArmour(ped, NekoAC.maxArmour or 100)
...
RegisterNUICallback('playerAction', function(data, cb)
    TriggerServerEvent('nekoac:adminAction', data or {})
    cb({ ok = true })
end)
    ...
    <main id="app" class="hidden">
      ...
      <section class="grid">
        ...
        <article class="panel action-panel">
          <div class="title">PLAYER ACTIONS <small id="actionState">select a player</small></div>
          <div id="actions"></div>
        </article>
      </section>
...
let current={players:[]};
let selectedId=null;
const res=(name,data={})=>fetch('https://neko-anticheat/'+name,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)});
document.getElementById('close').onclick=()=>res('close');
document.getElementById('refresh').onclick=()=>res('refresh');
document.getElementById('stopSpectate').onclick=()=>res('stopSpectate');
document.getElementById('clearIncidents').onclick=()=>res('clearIncidents');
document.querySelectorAll('[data-profile]').forEach(btn=>btn.onclick=()=>res('profile',{profile:btn.dataset.profile}));
window.addEventListener('message',event=>{const msg=event.data||{};if(msg.type==='open'){app.classList.remove('hidden');res('refresh')}if(msg.type==='close')app.classList.add('hidden');if(msg.type==='data')render(msg.payload||{})});
function render(data){
  ...
  document.getElementById('playersCount').textContent=list.length;
  document.getElementById('incidentCount').textContent=data.incidentCount||0;
  document.getElementById('banCount').textContent=data.banCount||0;
  document.getElementById('guardCount').textContent=Object.keys(data.protectedResources||{}).length;
  document.querySelectorAll('[data-profile]').forEach(btn=>btn.classList.toggle('active',btn.dataset.profile===data.profile));
  players.innerHTML=list.map(p=>'<div class="player '+(Number(p.id)===Number(selectedId)?'selected':'')+'" data-id="'+p.id+'"><span class="id">#'+p.id+'</span><b>'+esc(p.name)+'<small>'+(p.ping||'--')+' ms ping // '+(p.inventoryCount||0)+' items // '+((p.flags||[]).length)+' flags</small></b><span class="score">score '+(p.score||0)+'</span></div>').join('')||'<div class="empty">No live players.</div>';
  players.querySelectorAll('[data-id]').forEach(row=>row.onclick=()=>{selectedId=Number(row.dataset.id);render(current)});
  const selected=list.find(p=>Number(p.id)===Number(selectedId));
  renderInspector(selected);
  renderActions(selected);
}
function renderInspector(p){
  if(!p){
    selectedState.textContent='no target selected';
    inspector.className='empty';
    inspector.innerHTML='Select a player to view identifiers, telemetry, and inventory.';
    return;
  }
  const t=p.telemetry||{};
  const coords=t.coords?('X '+fmt(t.coords.x)+' / Y '+fmt(t.coords.y)+' / Z '+fmt(t.coords.z)):'Waiting for heartbeat';
  selectedState.textContent='#'+p.id+' // '+(p.ping||'--')+' ms';
  inspector.className='';
  inspector.innerHTML='<div class="inspect-hero"><h2>'+esc(p.name)+'</h2><p>Server ID #'+p.id+' // '+(p.ping||'--')+' ms ping // score '+(p.score||0)+'</p></div><div class="kv-grid"><div><span>HEALTH</span><strong>'+safe(t.health)+'</strong></div><div><span>ARMOUR</span><strong>'+safe(t.armour)+'</strong></div><div><span>SPEED</span><strong>'+fmt(t.speed)+'</strong></div><div><span>VISIBLE</span><strong>'+yes(t.visible)+'</strong></div><div><span>VEHICLE</span><strong>'+yes(t.inVehicle)+'</strong></div><div><span>DEAD</span><strong>'+yes(t.dead)+'</strong></div><div><span>WEAPON</span><strong>'+safe(t.weapon)+'</strong></div><div><span>LAST HEARTBEAT</span><strong>'+safe(p.lastHeartbeat)+'</strong></div><div><span>POSITION</span><strong>'+esc(coords)+'</strong></div></div><div class="subhead">INVENTORY SNAPSHOT</div><div class="inventory">'+((p.inventory||[]).slice(0,12).map(item=>'<div><span>'+esc(item.name||item.label||'item')+'</span><strong>x'+(item.count||1)+'</strong></div>').join('')||'<div><span>NO ITEMS</span><strong>Framework inventory empty</strong></div>')+'</div><div class="subhead">IDENTIFIERS</div>'+((p.identifiers||[]).map(id=>'<div class="identifier">'+esc(id)+'</div>').join('')||'<div class="identifier">No identifiers exposed</div>');
}
function renderActions(p){
  if(!p){
    actionState.textContent='select a player';
    actions.innerHTML='<div class="empty">Choose a player from Live Players to unlock staff actions.</div>';
    return;
  }
  actionState.textContent=esc(p.name);
  const items=[['spectate','Spectate','Watch this player from your staff client',false],['stopSpectate','Stop spectate','Return your client camera to normal',false],['clearFlags','Clear flags','Reset this player score and warning cache',false],['freeze','Freeze','Freeze this player in place',false],['unfreeze','Unfreeze','Release movement lock',false],['heal','Heal','Restore health and armour',false],['slay','Slay','Set player health to zero',true],['kick','Kick','Remove player from the server',true],['ban','Ban','Add a local Neko ban and drop player',true]];
  actions.innerHTML='<div class="action-grid">'+items.map(item=>'<div class="action-card"><b>'+item[1]+'<small>'+item[2]+'</small></b><button class="'+(item[3]?'danger':'')+'" data-action="'+item[0]+'">'+item[1].toUpperCase()+'</button></div>').join('')+'</div>';
  actions.querySelectorAll('[data-action]').forEach(btn=>btn.onclick=()=>{const action=btn.dataset.action;if(action==='spectate')return res('spectate',{target:Number(p.id)});if(action==='stopSpectate')return res('stopSpectate');res('playerAction',{target:Number(p.id),action})});
}
function esc(value){return String(value??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
function safe(v){return v===undefined||v===null||v===''?'--':esc(v)}
function fmt(v){return typeof v==='number'?v.toFixed(2):safe(v)}
...
RegisterNetEvent('nekoac:adminAction', function(payload)
    local src = source
    if src <= 0 then return end
    payload = payload or {}
    local target = tonumber(payload.target)
    local action = tostring(payload.action or '')
    if not target or not playerOnline(target) then
        TriggerClientEvent('chat:addMessage', src, { args = { 'NekoAC', 'Selected player is not online.' } })
        return
    end
    local player = ensurePlayer(target)
    if action == 'clearFlags' then
        player.flags = {}
        player.score = 0
    elseif action == 'freeze' or action == 'unfreeze' or action == 'heal' or action == 'slay' then
        TriggerClientEvent('nekoac:clientAction', target, action)
    elseif action == 'kick' then
        DropPlayer(target, 'Kicked by Neko Anti-Cheat staff action.')
    elseif action == 'ban' then
        local reason = 'Banned by Neko Anti-Cheat staff action.'
        bans[#bans + 1] = {
            name = player.name,
            identifiers = player.identifiers,
            reason = reason,
            module = 'IDENTITY_SIGNALS',
            severity = 100,
            createdAt = os.date('!%Y-%m-%dT%H:%M:%SZ'),
            provider = 'Neko Anti-Cheat'
        }
...
`;
  ...
  return { manifest, config, client, server, guard, uiHtml, uiCss: uiCssFinal, uiJs, readme };
}
